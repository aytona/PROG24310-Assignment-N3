...
Simulation::~Simulation() {
    // Need to delete all the objects created in the generateCustomer
    for (int i = m_premCustomers.size() - 1; i > 0; --i) {
        delete m_premCustomers.at(i);
    }
    
    for (int i = m_regCustomers.size() - 1; i > 0; --i) {
        delete m_regCustomers.at(i);
    }
    
    m_premCustomers.clear();
    m_regCustomers.clear();
}
...
void Simulation::printResult() {
	...
	} else if (premSmallest.getBalance() > this->m_premCustomers.at(i - 1)->getBalance()) {
